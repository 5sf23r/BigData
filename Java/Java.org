...
** static关键字(静态)
   ...
   - 修饰变量
     ...
     静态变量随着类的加载就加载到了方法区，并且会自动赋初始值
     静态变量有限于对象存在，可以通过类名来调用，静态变量被所有对象共享。
   ...
   - 修饰方法
     ...
     注意：
         静态变量不可以定义到静态方法中
         静态方法中，不可以直接调用非静态的方法。另外也不能存在this和super这样的关键字。讲台是优先于对象存在的。
         静态方法可以重载
         静态方法不可以重写，但是可以存在方法签名一样的静态方法，这种方式叫做隐藏
         隐藏时需注意：父类方法和子类方法要么都是静态的，要么都是非静态的
   - 修饰代码块
     用static修饰的代码块
     它是随着类的加载而加载到方法区中，在类创建对象前去执行一次，而且也只会在类
** final
