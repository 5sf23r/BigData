...
public class LoginServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //1. 获取页面中的form 表单的数据：username，password，remname
        ...
        //2. 判断remname 是否勾选了，是否为null
        if (WebUtils.isNull(remname)) {
            ////2.1 如果为null，表示【不记住用户名】，
            //删除原有的cookie 或者 如果没有，不需要生成cookie
            Cookie cookie = new Cookie("username","");
            cookie.setMaxAge(0);
            response.addCookie(cookie);
        } else {
            ////2.2 如果不为null，表示【记住用户名】，
            //原来有cookie，需要把username重新赋值给cookie
            //原来没有cookie，需要生成一个新的cookie，响应给浏览器
            Cookie cookie = new Cookie("username","");
        ...
        try {
            ...
            rs = ps.executeQuery();
            ////3.1 如果存在，表示登录成功，重定向到【主界面index.jsp】
            if (rs.next()) {
                //使用session对象把用户名绑定
                request.getSession().setAttribute("username",username);
                response.sendRedirect("/index.jsp");
            } else {
                ////3.2 如果不存在，表示登录失败，请求转发到【login.jsp】，给出提示信息
